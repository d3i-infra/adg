...
|---|---|---|
| npm | `@d3i-infra/adg` (launcher) + `@d3i-infra/adg-{darwin,linux,win32}-{x64,arm64}` (one binary each), assembled by `scripts/npm/build.mjs` from goreleaser's build output | `NPM_TOKEN` (granular automation token on the `d3i-infra` npm org) |
...
fails, the GitHub Release has already been published; fix the secret and re-run the workflow from
the Actions tab (npm refuses to republish an existing version, so re-runs after a partial npm
