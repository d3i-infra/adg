...
3. The `release` workflow runs goreleaser and npm, publishing all four outputs: the GitHub
   ...
   tags, and for every tag while the AUR route is dormant: see "Packagers" below); and the seven
   npm packages (`@d3i-infra/adg` plus six platform packages, dist-tag `next` for prereleases,
   `latest` otherwise).
4. Verify: the Release page lists all 13 assets, and a downloaded asset prints `X.Y.Z`:
   ...
   for a prerelease tag, which the AUR never receives, and while the AUR route is dormant: the
   goreleaser log then shows the AUR push skipped with `private_key is empty`).
...
| AUR | `adg-bin`, a PKGBUILD generated by goreleaser from the tar.gz archives | `AUR_SSH_PRIVATE_KEY` — **unset for now** (AUR account registration closed, 2026-09) |
...
**The AUR route is dormant.** With `AUR_SSH_PRIVATE_KEY` unset the workflow passes goreleaser an empty
string, goreleaser skips the push (`private_key is empty`), and the release succeeds. Do not set the
secret before its public half is registered on an AUR account: a rejected push fails the goreleaser
step, and the npm steps after it never run. Once the account exists and the secret is set, the next
non-prerelease tag publishes `adg-bin`; in that same release remove the "not yet available" notes
(README install table, `docs/lean-example/hook-setup.md`, `tools/adr-plugin/README.md`, the
session-start install string and its test), this paragraph, and the two dormant-route qualifiers in
"Cutting a release" steps 3 and 4.
